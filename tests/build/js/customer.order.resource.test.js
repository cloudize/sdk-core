const {
  ...
  MockRestClient,
} = require('@apigames/rest-client');
const dateUtils = require('date-and-time');
const { ResourceFilterType, SDKException } = require('../../../lib');
const {
  CustomerOrders,
  Order,
...
} = require('./customer.order.resource');
  ...
  describe('container\'s Count() method ', () => {
    it('should throw an error if an invalid payload is received.', async () => {
      ...
      try {
        ...
        expect(error.code).toBe('COUNT-FAILED');
        expect(error.message).toBe('The request to count the number of resources related '
    ...
    it('should return the document count returned by the API.', async () => {
      ...
      const customerOrders = new CustomerOrders('9a383573-801f-4466-80b2-96f4fb93c384', mockClient);
      const count = await customerOrders.Filter(OrderFilterProductCode, ResourceFilterType.Equal, 'abc')
        .Sort(OrderSortOrderDate)
        .Include(OrderIncludeCustomer)
        .PageNumber(2, 10)
        .Count();
    ...
    it('should return the cached document count for subsequent queries.', async () => {
      ...
      const firstCount = await customerOrders.Filter(OrderFilterProductCode, ResourceFilterType.Equal, 'abc')
        .Sort(OrderSortOrderDate)
        .Include(OrderIncludeCustomer)
        .PageNumber(2, 10)
        .Count();

      const secondCount = await customerOrders.Filter(OrderFilterProductCode, ResourceFilterType.Equal, 'abc')
        .Sort(OrderSortOrderDate)
        .Include(OrderIncludeCustomer)
        .PageNumber(2, 10)
        .Count();
  ...
  describe('container\'s Get() method ', () => {
    ...
    it('should throw errors returned by the API.', async () => {
      ...
      try {
        ...
        expect(error.errors[0].code).toBe('404-NOT-FOUND');
        expect(error.errors[0].title).toBe('The requested resource was not found.');
        expect(error.errors[0].status).toBe(404);
        expect(error.errors[1].code).toBe('405-STILL-NOT-FOUND');
        expect(error.errors[1].title).toBe('The requested resource was still not found.');
        expect(error.errors[1].status).toBe(405);
      }
    });
  ...
  describe('container\'s Find() method ', () => {
    ...
    it('should throw errors returned by the API.', async () => {
      ...
      try {
        ...
        expect(error.errors[0].title).toBe('The requested resource was not found.');
        expect(error.errors[0].status).toBe(404);
        expect(error.errors[1].code).toBe('404-STILL-NOT-FOUND');
        expect(error.errors[1].title).toBe('The requested resource was still not found.');
        expect(error.errors[1].status).toBe(404);
        expect(error.errors[1].detail).toBe('After an extensive look around the '
          + 'database, nothing resembling the desired resource was found.');
        expect(error.errors[1].source)
          .toEqual({ database: { collections: 'all of them really' } });
      }
    });

    it('should clear the internal structures, get the resource (by page number) and repopulate the internal '
        + 'data structures (ignoring extra fields).', async () => {
      const mockClient = new MockRestClient();
      jest.spyOn(mockClient, 'Get');
      mockClient.MockResolve({
        statusCode: 200,
      ...
      const customerOrders = new CustomerOrders('9a383573-801f-4466-80b2-96f4fb93c384', mockClient);
      await customerOrders.Filter(OrderFilterProductCode, ResourceFilterType.Equal, 'abc')
        .Sort(OrderSortOrderDate)
        .Include(OrderIncludeCustomer)
        .PageNumber(2, 10)
        .Find();
    ...
    it('should clear the internal structures, get the resource (by page offset) and repopulate the internal '
  ...
  describe('container\'s Save() method ', () => {
    ...
    it('should throw an error if the API returns an error during the operation.', async () => {
      ...
      try {
        ...
        expect(error.errors[0].code).toBe('409-CONFLICT');
        expect(error.errors[0].title).toBe('A conflict occurred.');
        expect(error.errors[0].status).toBe(409);
      }
    });
    ...
    it('should throw an error if the POST operation does not return the resource location.', async () => {
      ...
      try {
        ...
        expect(error).toBeDefined();
        expect(error).toBeInstanceOf(SDKException);
        expect(error.code).toBe('INVALID-LOCATION');
        expect(error.message).toBe('The save operation was unable to retrieve the location of '
    ...
    it('should throw an error if the POST operation does not return the resource id.', async () => {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(SDKException);
        expect(error.code).toBe('INVALID-RESOURCE-ID');
        expect(error.message).toBe('The save operation was unable to retrieve the identifier of '
            + 'the resource created by the API.');
      }
    });
